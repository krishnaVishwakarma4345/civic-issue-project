...
export default function ReportIssuePage() {
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 2 && !location) {
      setSubmitError("Please capture or enter your location before continuing.");
      return;
    }
  ...
  const onSubmit = async (data: CreateIssueFormData) => {
    if (!location) {
      setSubmitError("Location is required. Please capture your location.");
      setCurrentStep(2);
      return;
    }
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center max-w-sm mx-auto">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Your issue has been submitted successfully. Redirecting to issue
            details...
          </p>
                  ...
                  {(() => {
                    ...
                    return cat ? (
                      <p className="text-xs text-blue-700">
                        <span className="font-semibold">{cat.icon} {cat.label}:</span>{" "}
                        {cat.description}
                      </p>
                    ) : null;
                  })()}
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 rounded-xl border-2 border-dashed border-primary-200 bg-primary-50/50 text-center">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  We'll automatically fill in your GPS coordinates and address
                </p>
                <Button
                  type="button"
                  variant="primary"
              ...
              <Input
                ...
                error={!location && manualAddress === "" ? errors.address?.message : undefined}
                required
        ...
        {currentStep === 3 && (
          <Card>
            ...
            <p className="text-sm text-gray-500 mb-4">
              Photos help authorities verify and prioritise your issue faster.
        ...
        {currentStep === 4 && (
          <Card>
            ...
            <div className="space-y-4">
                ...
                <ReviewRow
                  ...
                  value={
                    CATEGORIES.find((c) => c.value === watchedValues.category)
                      ?.label ?? watchedValues.category
                  }
                />
                <ReviewRow
                  label="Priority"
                  value={
                    PRIORITIES.find((p) => p.value === watchedValues.priority)
                      ?.label ?? watchedValues.priority
                  }
                />
                <ReviewRow
                  label="Location"
                  value={location?.address ?? "Not captured"}
                />
                <ReviewRow
                  label="Photos"
                  value={
                    imageFiles.length > 0
                      ? `${imageFiles.length} photo${imageFiles.length !== 1 ? "s" : ""} attached`
                      : "No photos attached"
                  }
                />
              </div>
              ...
              {/* Info */}
              <Alert variant="info">
                By submitting, this issue will be assigned to the relevant
                municipal department for review and resolution. You'll receive
                notifications as it progresses.
              </Alert>
            </div>
          </Card>
        )}
